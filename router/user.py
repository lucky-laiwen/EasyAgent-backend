...
async def get_user_by_id(user_id: int, db: Session = Depends(get_db),_: str = Depends(bearer_token)):
    return crud_user.get_user(db=db, user_id=user_id)


@router.get("/query_All", response_model=list[UserOut])
async def get_all_users(db: Session = Depends(get_db),_: str = Depends(bearer_token)):
    return crud_user.get_all_users(db)


@router.post("/create_user", response_model=UserSchema, status_code=status.HTTP_201_CREATED)
async def create_user(user: UserCreate, db: Session = Depends(get_db),_: str = Depends(bearer_token)):
    try:
